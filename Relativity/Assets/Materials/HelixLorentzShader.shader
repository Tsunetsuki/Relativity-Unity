Shader "Unlit/HelixLorentz"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float R = 0.5;
            float3 h(float t) {
                return float3(R*cos(t), R*sin(t), t*t);
            }

            float3 hp(float t) {
                return float3(-R*sin(t), R*cos(t), t);
            }

            float3 FindRootNewton(float x, float3 point_on_plane, float3 plane_normal) { // , float-> float3 f, flolat -> float3 fp
                float epsilon = 0.00001;
                float xn = x + 2 * epsilon;

                while(abs(xn - x) > epsilon) {
                    x = xn;
                    float f = dot(h(x), plane_normal)- dot(point_on_plane, plane_normal);
                    float fp = dot(hp(x), plane_normal);
                    xn = x - f / fp;
                }
                return h(xn);
            }
            ...
            v2f vert(appdata v)
            {
                ...
                float3 projectedVertexPos = FindRootNewton(0, _ObserverPos, _ObserverVel);
