...
public class ObserveeObjectHelix: MonoBehaviour
{
    ...
    Vector3 h(float t) {
        return new Vector3(R * Mathf.Cos(t), R * Mathf.Sin(t), t * t);
    }

    Vector3 hp(float t) {
        return new Vector3(-R * Mathf.Sin(t), R * Mathf.Cos(t), t);
    }
    ...
    private void FixedUpdate() {
        if (!IsCopy)
        {
            ...
            transform.position = FindRootNewton(observer.transform.position, Vector3.up);
        }
        else
        {
            throw new System.Exception("Copy unimplemented");
